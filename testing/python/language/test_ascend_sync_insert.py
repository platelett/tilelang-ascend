...
def _events(function):
    return [str(call.args[0].value) for call in _calls(function) if call.op == tvm.ir.Op.get("tl.ascend_auto_set_flag")]


def _assert_handoff_before_last_copy(function, event):
    calls = _calls(function)
    assert str(calls[-1].args[0].value) == "copy_ub_to_ub"
    positions = {}
    for kind in ("set", "wait"):
        matches = [
            (index, int(call.args[1]))
            for index, call in enumerate(calls)
            if call.op == tvm.ir.Op.get(f"tl.ascend_auto_{kind}_flag") and str(call.args[0].value) == event
        ]
        assert len(matches) == 1
        positions[kind] = matches[0]
    assert positions["set"][0] < positions["wait"][0] < len(calls) - 1
    assert positions["set"][1] == positions["wait"][1]
...
def test_outstanding_reader_survives_another_pipe_read(destination, b_offset, needs_event):
    buffers = _buffers()
    function = _run(buffers, _reader_sequence(buffers, destination), b_offset)
    assert ("MTE3_V" in _events(function)) == needs_event
    if needs_event:
        _assert_handoff_before_last_copy(function, "MTE3_V")
